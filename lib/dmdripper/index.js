...
function ripDmdFrames() {
  return loadFile(romU06Path)
    ...
    .then((wpcSystem) => {
      ...
      const aa = getGameMemoryValue(wpcSystem.getUiState(), 'GAME_CURRENT_SCREEN');
      if (aa === undefined) {
      ...
      for (let x = 0; ypos < 9999; x++) {
        ...
        try {
          ...
          const currentScreen = getGameMemoryValue(state, 'GAME_CURRENT_SCREEN');
