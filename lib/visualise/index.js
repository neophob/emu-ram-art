...
const config = configObject.a3dpi600;
...
function drawDiagram(config, filteredData) {
  ...
  for (let i = 0; i < loopLength; i++) {
    ...
    if (correctAmountOfFilteredData.length > 330) {
      ...
      const filenameToDisplay = regexMatch && regexMatch.lenght > 1 ? regexMatch[1] : path.basename(jsonPath);
