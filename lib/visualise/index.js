...
for (let i = 0; i < loopLength; i++) {
  ...
  const correctAmountOfFilteredData = lib.filterSlots(dataWindow, config.diagramWidth, 0, config.maximalFlatLineFactor);
  ...
  if (correctAmountOfFilteredData.length) {
    ...
    render.render(correctAmountOfFilteredData, filename, headerText);
  } else {
