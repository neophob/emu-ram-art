 ...
 */
function fillSlots(data, slotSize, offset, maximalFlatlineFactor) {
  ...
  let result = sliceData(offset, slotSize);
  const loopLength = Math.floor(data.length/slotSize) - 1;
  for (let i = 0; i < loopLength; i++) {
    offset += slotSize;
    result = result.concat(sliceData(offset, slotSize));
  }
