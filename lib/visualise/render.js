...
function render(data, config, renderMetaData) {
  ...
  const initialYPos = config.borderSize + config.diagramHeight - 240;
  let count = 0;
  ...
  filteredMemory
    .forEach((frame) => {
      ...
      if (y >= (config.imageHeight - config.borderSize - 200)) {
        y = initialYPos;
  ...
  ctx.fillText(
    ...
    textXpos,
    config.imageHeight - config.borderSize - 240 - config.fontSize
  );
  ctx.fillText(
    renderMetaData.durationText,
    textXpos,
    config.imageHeight - config.borderSize - 220
  );
