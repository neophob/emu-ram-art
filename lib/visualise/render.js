...
function render(filteredMemory, config, renderMetaData) {
  ...
  ctx.fillRect(textXpos, config.imageHeight - config.borderSize - config.fontSize * 2.5, config.diagramWidth * config.widthScaleFactor, config.diagramHeight);
  ctx.fillStyle = config.fontFillStyle;
  ctx.fillText(renderMetaData.filenameToDisplay, textXpos, config.imageHeight - config.borderSize - config.fontSize * 2.5);
  ctx.fillText(renderMetaData.durationText, textXpos, config.imageHeight - config.borderSize - config.fontSize * 1.5);
