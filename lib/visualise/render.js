...
function render(data, config, renderMetaData) {
  ...
  const initialYPos = config.borderSize + config.diagramHeight - EMPTY_SPACE_PX - 40;
  let count = 0;
  ...
  ctx.fillText(
    ...
    textXpos,
    config.imageHeight - config.borderSize - EMPTY_SPACE_PX - 40 - config.fontSize
  );
  ctx.fillText(
    renderMetaData.durationText,
    textXpos,
    config.imageHeight - config.borderSize - EMPTY_SPACE_PX - 20
  );
