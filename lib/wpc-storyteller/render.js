...
function render(col, flags) {
  ...
  ctx.fillText('Condition Code Register Map', xOfs+config.xOfsAdd, yOfs - blockSize/2);
    ...
    flags.forEach((entry, index) => {
      ...
      if (i==7) {
        ...
        if (timing !== lastTiming && value) {
          ...
          console.log(timing, xOfs+config.textXOfs, yOfs + blockSize/2)
          lastTiming = timing;
        }
      }
